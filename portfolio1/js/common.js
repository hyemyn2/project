...
// index&sub page의 경로가 다를 경우, 절대 경로 잡기 : / 놓기
    ...
    $('footer').load('/portfolio1/inc_head_foot.html footer .foot');
    ...
    function head(){
        ...
        tie.addEventListener('mouseover',showSub);
    ...
    function checkValue(){
        ...
        if(!idCheck.test(inputID.value)){
            feedback('아이디를 다시 입력해주세요.','id'); return;
        } else if(!pwCheck.test(inputPW.value)){
            feedback('비밀번호를 다시 입력해주세요.','pw'); return;
        } else{
            clickLogoff();
        }
    }
